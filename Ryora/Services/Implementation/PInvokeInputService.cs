...
{
    public class PInvokeInputService : IInputService
    {
        /// <summary>
        ...
        public void SendKeys(bool isDown, short[] keys)
        {
            var inputList = Keyboard.Handle(isDown, keys);
            SendInput((uint)inputList.Length, inputList, INPUT.Size);
        }
        ...
        private class MouseHandler
        {
            ...
            private bool GetButtonState(MouseButton button)
            {
                return (button == MouseButton.Right
                    ? IsRightButtonDown
                    : button == MouseButton.Middle ? IsMiddleButtonDown : IsLeftButtonDown);
            }
            ...
            private INPUT? HandleMouseButton(MouseButton button, bool isDown)
            {
                            ...
                            mi = new MOUSEINPUT()
                            {
                                ...
                                dwFlags =
                                    (button == MouseButton.Right
                                        ? MOUSEEVENTF.RIGHTUP
                                        : button == MouseButton.Middle ? MOUSEEVENTF.MIDDLEUP : MOUSEEVENTF.LEFTUP),
                                time = 0,
                                dwExtraInfo = UIntPtr.Zero
                            }
                        }
        ...
        private class KeyboardHandler
        {
            ...
            private INPUT AddKey(VirtualKeyShort key, bool isDown)
            {
                return new INPUT()
                {
